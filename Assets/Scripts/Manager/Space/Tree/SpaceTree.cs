...
public class SpaceTree
{
    ...
    public void Remove(SpaceItem spaceItem)
    {
        ...
        if (node != null)
            ...
            Debug.LogWarning($"【空间八叉树】{spaceItem.spaceId}移除失败，在树中未找到");
    ...
    public void UpdateItem(SpaceItem spaceItem)
    {
        ...
        if (node == null)
        {
            Debug.LogWarning($"【空间八叉树】{spaceItem.spaceId}更新失败，原节点查找失败");
            return;
        }
        ...
        if (node.bound.IsContains(spaceItem.bound))
            ...
            {
                _itemDic.Remove(spaceItem.spaceId);
                Debug.LogWarning($"【空间八叉树】{spaceItem.spaceId}更新失败，新位置插入失败");
                spaceItem.SetSpaceID(-1);
                return;
            }
        _itemDic[spaceItem.spaceId] = new(newItem.bound, spaceItem);
    }
    public bool RayCast(Space.Ray ray, out SpaceItem hit, out float distance)
    {
        ...
        if (isHit)
        {
            ...
            if (_itemDic.TryGetValue(treeItem.id, out var dicItem))
                ...
                Debug.LogError($"【空间八叉树】{treeItem.id}字典中查找失败");
        }
        else
            distance = -1;
        hit = hitItem;
    ...
    private SpaceTreeNode GetNode(TreeItem item)
    {
        if (!_root.bound.IsContains(item.bound))
        {
            Debug.LogWarning($"【空间八叉树】超出树边界，无效Item{item.id}");
            return null;
        }
